...
Account::Account(int id, const string& name, double initialBalance, const string curr, string username, string password)
    : accountId(id), accountHolderName(name), balance(initialBalance), 
      currency(curr),  userName(username), passWord(password), isActive(true){
    regex specialChars(R"([a-zA-Z])");
    if (regex_search(accountHolderName, specialChars) || accountHolderName=="" || initialBalance < 0 || id<=0)
        throw invalid_argument("error");
    
}
Account::Account(int id, const string& name, string username, string password)
    : accountId(id), accountHolderName(name), balance(0), 
      currency("CAD"), userName(username), passWord(password), isActive(true) {
    regex specialChars(R"([a-zA-Z])");
